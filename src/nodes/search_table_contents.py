...
class SearchTableContentsNode:
    ...
    def execute(state: dict) -> dict:
        try:
            return SearchTableContentsNode._run(state)
        except Exception as e:
            state["status"] = "failed"
            state["error_node"] = "search_table_contents"
            state["error"] = str(e)
            print(f"❌ Error en SearchTableContentsNode: {e}")
            return state
    ...
    def _run(state: dict) -> dict:
        ...
        try:
            ...
            if doc_id and toc_data.get("structure"):
                SearchTableContentsNode._save_to_db(doc_id, toc_data["structure"])
