    ...
    def analyze_image(self, image_b64: str, prompt: str, system_prompt: str, config: Dict[str, Any]) -> Tuple[Any, str, int, int]:
        ...
        try:
            ...
            resp = self.client.chat.completions.create(
                ...
                temperature=temperature,
                timeout=timeout,
                response_format={"type": "json_object"} if "json" in prompt.lower() or "json" in system_prompt.lower() else None
            )
            dt = time.time() - t0
            ...
            try:
                data = json.loads(raw)
                elementos = data.get('elements', data.get('elementos', [])) # Soporta ingles (elements) y español (elementos)
